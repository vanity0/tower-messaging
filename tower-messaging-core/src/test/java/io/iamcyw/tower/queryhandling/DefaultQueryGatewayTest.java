...
import io.iamcyw.tower.messaging.responsetypes.InstanceResponseType;
import io.iamcyw.tower.utils.MockException;
...
import static io.iamcyw.tower.messaging.responsetypes.ResponseTypes.instanceOf;
...
class DefaultQueryGatewayTest {
    ...
    void testPointToPointQuery() throws Exception {
        ...
        assertTrue(InstanceResponseType.class.isAssignableFrom(result.getResponseType()
                                                                       .getClass()));
        assertEquals(String.class, result.getResponseType()
    ...
    void testPointToPointQueryWithMetaData() throws Exception {
        ...
        GenericMessage<String> testQuery = new GenericMessage<>("query", MetaData.with(expectedMetaDataKey,
    ...
    void testPointToPointQueryWhenQueryBusReportsAnError() throws Exception {
        ...
        when(mockBus.query(anyMessage(String.class, String.class))).thenReturn(
                completedFuture(new GenericQueryResponseMessage<>(String.class, expected)));

        CompletableFuture<String> result = testSubject.query("query", String.class);

        assertTrue(result.isDone());
        assertTrue(result.isCompletedExceptionally());
        assertEquals(expected.getMessage(), result.exceptionally(Throwable::getMessage)
                .get());
    }
    ...
    void testScatterGatherQuery() {
        ...
        when(mockBus.scatterGather(anyMessage(String.class, String.class), anyLong(), any())).thenReturn(
                Stream.of(answer));
    ...
    void testScatterGatherQuerySpecifyingQueryName() {
        ...
        Stream<String> queryResponse = testSubject.scatterGather(expectedQueryName, "scatterGather",
                                                                 instanceOf(String.class), expectedTimeout,
                                                                 expectedTimeUnit);
        Optional<String> firstResult = queryResponse.findFirst();
        assertTrue(firstResult.isPresent());
        assertEquals("answer", firstResult.get());
    ...
    void testScatterGatherQueryWithMetaData() {
        ...
        GenericMessage<String> testQuery = new GenericMessage<>("scatterGather", MetaData.with(expectedMetaDataKey,
                                                                                               expectedMetaDataValue));
    ...
    void testSubscriptionQuery() {
        when(mockBus.subscriptionQuery(any(), anyInt())).thenReturn(
                new DefaultSubscriptionQueryResult<>(Mono.empty(), Flux.empty(), () -> true));

        testSubject.subscriptionQuery("subscription", instanceOf(String.class), instanceOf(String.class));

        //noinspection unchecked
        ArgumentCaptor<SubscriptionQueryMessage<String, String, String>> queryMessageCaptor = ArgumentCaptor.forClass(
                SubscriptionQueryMessage.class);
        ...
        assertEquals(String.class, result.getResponseType()
                .getExpectedResponseType());
        assertTrue(InstanceResponseType.class.isAssignableFrom(result.getUpdateResponseType()
                                                                       .getClass()));
        assertEquals(String.class, result.getUpdateResponseType()
                .getExpectedResponseType());
        assertEquals(MetaData.emptyInstance(), result.getMetaData());
    }
    ...
    void testSubscriptionQueryWithMetaData() {
        ...
        GenericMessage<String> testQuery = new GenericMessage<>("subscription", MetaData.with(expectedMetaDataKey,
                                                                                              expectedMetaDataValue));
    ...
    void testDispatchInterceptor() {
        ...
        testSubject.query("query", String.class)
                .join();
    ...
    void testExceptionInInitialResultOfSubscriptionQueryReportedInMono() {
        when(mockBus.subscriptionQuery(anySubscriptionMessage(String.class, String.class), anyInt())).thenReturn(
                new DefaultSubscriptionQueryResult<>(
                        Mono.just(new GenericQueryResponseMessage<>(String.class, new MockException())), Flux.empty(),
                        () -> true));
        ...
        assertEquals(MockException.class, actual.initialResult()
                .map(i -> null)
                .onErrorResume(e -> Mono.just(e.getClass()))
                .block());
    }
    ...
    void testNullInitialResultOfSubscriptionQueryReportedAsEmptyMono() {
        ...
        assertNull(actual.initialResult()
                           .block());
    }
    ...
    void testNullUpdatesOfSubscriptionQuerySkipped() {
        when(mockBus.subscriptionQuery(anySubscriptionMessage(String.class, String.class), anyInt())).thenReturn(
                new DefaultSubscriptionQueryResult<>(Mono.empty(), Flux.just(
                        new GenericSubscriptionQueryUpdateMessage<>(String.class, null)), () -> true));
        ...
        assertNull(actual.initialResult()
                           .block());
        assertEquals((Long) 0L, actual.updates()
                .count()
                .block());
    }
    ...
    void testPayloadExtractionProblemsReportedInException() throws ExecutionException, InterruptedException {
        when(mockBus.query(anyMessage(String.class, String.class))).thenReturn(
                completedFuture(new GenericQueryResponseMessage<String>("test") {
                    @Override
        ...
        assertEquals("Faking serialization problem", actual.exceptionally(Throwable::getMessage)
                .get());
    }
